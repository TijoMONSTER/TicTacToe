...
//  TicTacToe
//
//  Created by Iván Mervich on 7/27/14.
...
- (void)viewDidLoad {
    ...
    self.myURLTextField.text = @"http://en.wikipedia.org/wiki/Tic-tac-toe";
    [self loadURLRequestWithString:self.myURLTextField.text];

}

- (void)loadURLRequestWithString:(NSString *)string {
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    [self loadURLRequestWithString:textField.text];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (scrollView.contentOffset.y < self.lastOffsetY) {
        // show textfield
        if (urlTextField.frame.origin.y < 20) {
            ...
            closeButton.frame = CGRectMake(closeButton.frame.origin.x, closeButton.frame.origin.y + 4, closeButton.frame.size.width, closeButton.frame.size.height);
        }


    } else {
        // hide textfield
        if (urlTextField.frame.origin.y > -30) {
            urlTextField.frame = CGRectMake(urlTextField.frame.origin.x, urlTextField.frame.origin.y - 4, urlTextField.frame.size.width, urlTextField.frame.size.height);
            clearUrlButton.frame = CGRectMake(clearUrlButton.frame.origin.x, clearUrlButton.frame.origin.y - 4, clearUrlButton.frame.size.width, clearUrlButton.frame.size.height);
            closeButton.frame = CGRectMake(closeButton.frame.origin.x, closeButton.frame.origin.y - 4, closeButton.frame.size.width, closeButton.frame.size.height);
        }
